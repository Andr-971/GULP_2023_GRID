...
//---------------------------------------------------- Меню на тачскринах -----------------
    ...
    menuRoot.addEventListener("click", (e) => { // Обработчик на корень меню
    ...
    function closeAllMenuLevel(current = null) { 
        let parents = [];
        if (current) { // Если есть текущее меню
            ...
            while (currentParent) {
                ...
                if (currentParent.nodeName === "UL") parents.push(currentParent); // Отлавливаем по ul и пушим в массив
        ...
        Array.from(menuLevel).forEach((item) => {
            if (item !== current && !parents.includes(item)) { // Если это не текущее событие и не родитель текущего события
                item.classList.remove("is-active"); // удалить
                if (item.previousElementSibling.nodeName === "SPAN") { // Если это не текщее событие кнопки
